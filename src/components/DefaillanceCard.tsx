...
export function DefaillanceCard({ defaillance: d, expanded, onToggle }: { defaillance: Defaillance; expanded: boolean; onToggle: () => void }) {
  const moyenDisplay = d.cout_moyen || Math.round((d.cout_min + d.cout_max) / 2);
    ...
    <div className={`bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-sm hover:shadow-md transition-shadow border-l-4 ${BORDER_COLORS[d.gravite]}`}>
      <button onClick={onToggle} aria-expanded={expanded} className="w-full px-4 py-3.5 flex items-center gap-3 text-left hover:bg-slate-50/50 transition cursor-pointer">
        ...
        {d.peut_faire_soi_meme && (
          <span className="hidden sm:inline text-[10px] px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full font-semibold ring-1 ring-emerald-200">Faisable soi-même</span>
        )}
        <div className="flex items-baseline gap-1.5 whitespace-nowrap">
          <span className="text-base font-extrabold text-foreground tabular-nums">~{moyenDisplay} &euro;</span>
          <span className="text-[11px] text-muted tabular-nums">({d.cout_min}-{d.cout_max})</span>
        </div>
      ...
      <div className={`overflow-hidden transition-[max-height] duration-300 ${expanded ? "max-h-80" : "max-h-0"}`}>
        <div className="px-5 pb-5 border-t border-slate-100 pt-4 flex flex-col gap-3 bg-slate-50/40">
          ...
          {/* Détail pièce / main-d'œuvre */}
          {(d.cout_piece || d.cout_main_oeuvre) && (
            <div className="flex flex-wrap gap-3 text-xs">
              {d.cout_piece && (
                <span className="px-2.5 py-1 bg-white rounded-lg border border-slate-100 text-muted">
                  <strong className="text-foreground">{d.cout_piece}</strong>
                </span>
              )}
              {d.cout_main_oeuvre && (
                <span className="px-2.5 py-1 bg-white rounded-lg border border-slate-100 text-muted">
                  <strong className="text-foreground">{d.cout_main_oeuvre}</strong>
                </span>
              )}
            </div>
          )}
          ...
          <div className="flex items-center flex-wrap gap-2 text-xs text-slate-400">
            {d.code && <span className="font-mono bg-white px-2 py-0.5 rounded border border-slate-100">CT {d.code}</span>}
            {d.localisation && <span className="bg-white px-2 py-0.5 rounded border border-slate-100">{d.localisation}</span>}
            {d.peut_faire_soi_meme && <span className="sm:hidden px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded ring-1 ring-emerald-200 font-semibold">Faisable soi-même</span>}
          </div>
        </div>
      </div>
    </div>
